...
window.addEventListener('load', () => {
    ...
    axios.get('http://localhost:3000/products').then((products) => {
        ...
        products.data.forEach(product => {
            const productHtml = `
                <div id="album-${product.id}" class="card">
                    <h3>${product.title}</h3>
                    <div class="image-container">
                        <img class="prod-images" src=${product.imageUrl} alt="">
                    </div>
                    <div class="card__content">
                    <span class="product__price"><b>${product.description}<b></span>
                    </div>
...
document.addEventListener('click',(e)=>{
    ...
    if (e.target.className=='shop-item-button'){
        ...
        const prodId = Number(e.target.parentNode.parentNode.parentNode.parentNode.id.split('-')[1]);
        console.log(prodId);
    ...
    if (e.target.className=='cart-btn-bottom' || e.target.className=='cart-bottom' || e.target.className=='cart-holder'){
        axios.get('http://localhost:3000/cart').then(carProducts => {
            document.querySelector('.cart-number').innerText = carProducts.data.length;
